-- =====================================================
-- LANGUAGE AND GST COMPLIANCE MIGRATION (FIXED)
-- =====================================================
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_items_gst_rate') THEN
    ALTER TABLE items ADD CONSTRAINT chk_items_gst_rate CHECK (gst_rate >= 0 AND gst_rate <= 100);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_items_cess_rate') THEN
    ALTER TABLE items ADD CONSTRAINT chk_items_cess_rate CHECK (cess_rate >= 0 AND cess_rate <= 100);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_transactions_gst_amount') THEN
    ALTER TABLE transactions ADD CONSTRAINT chk_transactions_gst_amount CHECK (gst_amount >= 0);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_transactions_cess_amount') THEN
...
DROP TRIGGER IF EXISTS trigger_translations_updated_at ON translations;
...
DROP TRIGGER IF EXISTS trigger_e_way_bills_updated_at ON e_way_bills;
